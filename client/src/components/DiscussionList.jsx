...
import { MessageSquare, Plus, Stethoscope, ThumbsUp, User as UserIcon, Eye, Search } from 'lucide-react';
...
export function DiscussionList({ discussions, setDiscussions, categories, onViewDiscussion, onReload }) {
  ...
  const getAuthorName = (d) => {
    if (d.author?.firstName) return `${d.author.firstName} ${d.author.lastName}`;
    return d.authorName || 'Unknown';
  };

  const getAuthorRole = (d) => d.author?.role || d.authorRole || 'regular';

  const filtered = discussions.filter(d =>
    !searchQuery || d.title?.toLowerCase().includes(searchQuery.toLowerCase()) || d.content?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2 bg-linear-to-r from-blue-600 to-indigo-600"><Plus className="w-4 h-4" /> New Discussion</Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleCreate} disabled={submitting} className="bg-linear-to-r from-blue-600 to-indigo-600">
      ...
      <div className="relative">
        ...
        <Input placeholder="Search discussions..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9" />
      </div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <Card><CardContent className="py-16 text-center">
            ...
            <h3 className="text-slate-600 font-medium">No discussions yet</h3>
            <p className="text-slate-400 text-sm mt-1">Be the first to start a conversation!</p>
          </CardContent></Card>
        ) : filtered.map(d => (
          <Card key={d._id} className="cursor-pointer hover:shadow-md transition-all hover:-translate-y-0.5" onClick={() => onViewDiscussion(d)}>
            <CardContent className="p-5">
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <Badge variant="outline" className="text-xs">{d.category}</Badge>
                {getAuthorRole(d) === 'doctor' && (
                  <Badge className="text-xs bg-green-100 text-green-700 border-green-200"><Stethoscope className="w-3 h-3 mr-1" /> Doctor</Badge>
                )}
              </div>
              ...
              {d.symptoms?.length > 0 && (
                <div className="flex gap-1 flex-wrap mt-2">
                  {d.symptoms.slice(0, 3).map(s => <Badge key={s} variant="secondary" className="text-xs">{s}</Badge>)}
                  {d.symptoms.length > 3 && <Badge variant="secondary" className="text-xs">+{d.symptoms.length - 3}</Badge>}
                </div>
              )}
              <div className="flex items-center justify-between mt-4 text-xs text-slate-400">
                <div className="flex items-center gap-1">
                  {getAuthorRole(d) === 'doctor' ? <Stethoscope className="w-3 h-3" /> : <UserIcon className="w-3 h-3" />}
                  <span>{getAuthorName(d)}</span>
                </div>
                <div className="flex items-center gap-4">
                  <button onClick={e => handleLike(e, d._id)} className={`flex items-center gap-1 transition-colors ${d.isLiked ? 'text-blue-600' : 'hover:text-blue-600'}`}>
                    <ThumbsUp className="w-3.5 h-3.5" /> {d.likes || 0}
                  </button>
                  <span className="flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5" /> {d.replyCount || 0}</span>
